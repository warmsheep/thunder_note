...
#### 18.7.9 关键 UI 继续按 Android 对齐
- [ ] 底部导航只有 5 项：闪记 / 合集 / 联系人 / 收藏 / 我的；**没有**「搜索」tab
- [ ] 闪记列表页右上角有灰色矢量搜索图标，点击后展开页内搜索条
- [ ] 闪记列表页右上角 `+` 为文字加号入口；右下角快速捕获 FAB 是橙色圆角方形，SVG `+` 居中
- [ ] 底部导航图标为灰色线性 SVG，选中态变主色；不再是拟物 emoji 图标
- [ ] 「我的」页面顶部资料区是橙色渐变背景，头像、昵称、用户名/邮箱、个人简介、编辑入口在渐变区域内
- [ ] 「我的」页面有「修改密码」入口；「设置」页面不再展示「修改密码」
- [ ] 闪记消息页 composer：附件 / 拍照 / 录音 / 卡片 / 发送均为 SVG 矢量图标；左侧操作图标前景为黑色
- [ ] composer 默认输入框高度约 40px，与左右 36px 图标按钮视觉高度一致，不再明显过高
