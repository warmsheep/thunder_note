...
### 18.6 闪记会话微博风布局（W28-06）
> 仅闪记会话（mode=flash）应用，桌面 + 移动端都生效；联系人对话保持原 IM。
- [ ] 进入闪记会话：composer 在页面上方（header 下方）
- [ ] 消息列表在 composer 下方，最新消息紧贴 composer（视觉顶部）
- [ ] 向下滚动 = 看更老的消息
- [ ] 滚到底部（最老消息）附近自动触发加载更多历史
- [ ] 发送新消息 → 立即出现在视觉顶部（紧贴 composer）
- [ ] 已发送消息：当前用户消息靠右、对方消息靠左，与原行为一致
- [ ] 离开会话再回来：滚动位置恢复（sessionStorage）
- [ ] 进入联系人会话（/chat/contact/:peerUserId）：composer 仍在**底部**，最新消息在底部，与原 IM 完全一致
- [ ] 「新消息」悬浮按钮：在闪记模式下从顶部弹出 ⬆，提示「跳到最新」；联系人模式下仍是底部 ⬇
